...
class Client extends Dispatcher {
  constructor (url, {
    ...
    // counter of response body size
    this[kResponseBodySize] = 0
    // will contain response header content-length
    this[kResponseContentLength] = 0
...
class Parser {
  ...
  onHeadersComplete (statusCode, upgrade, shouldKeepAlive) {
    ...
    for (let n = 0; n < rawHeaders.length; n += 2) {
      ...
      if (!keepAlive && key.length === 10 && key.toString().toLowerCase() === 'keep-alive') {
        ...
        client[kResponseContentLength] = parseInt(val.toString(), 10)
  ...
  onBody (buf) {
    ...
    try {
      client[kResponseBodySize] += buf.length
...
function onSocketEnd () {
  if (util.mismatchResponseContentLength(this[kClient][kResponseContentLength], this[kClient][kResponseBodySize]))  {
    util.destroy(this, new ResponseContentLengthMismatchError('socket end'))
    return
  }

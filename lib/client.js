...
class Client extends Dispatcher {
  constructor (url, {
    ...
    // @todo rename strictContentLength to requestStrictContentLength?
...
const llhttp = new WebAssembly.Instance(mod, {
  env: {
    ...
    wasm_on_headers_complete: (p, statusCode, upgrade, shouldKeepAlive) => {
      assert.strictEqual(currentParser.ptr, p)
      console.log('wasm_on_headers_complete', statusCode, upgrade, shouldKeepAlive)
...
class Parser {
  ...
  onHeadersComplete (statusCode, upgrade, shouldKeepAlive) {
    ...
    // @todo refactor to reset function
    this.headers = []
    this.headersSize = 0
    // @todo this.responseBodySize = 0
    // @todo this.responseContentLength = 0

    // @todo refactor: move next block to extractMeaningfulHeaders or something
    let keepAlive
    let trailers
    let contentLength
    ...
    for (let n = 0; n < rawHeaders.length; n += 2) {
      ...
      if (!keepAlive && key.length === 10 && key.toString().toLowerCase() === 'keep-alive') {
        ...
        trailers = val
      } else if (!contentLength && key.length === 14 && key.toString().toLowerCase() === 'content-length') {
        this.responseContentLength = parseInt(val, 10) || 0
        console.log('AAAAA', this.responseContentLength)
        contentLength = true
        // @todo emit process warn if content-length is invalid
      }
    }
  ...
  onBody (buf) {
    ...
    this.responseBodySize += buf.length
  ...
  onMessageComplete () {
    ...
    // @todo check response content length
    console.log('deeeeeeeee', this.responseBodySize, this.responseContentLength)
    if (util.mismatchContentLength(this.responseBodySize, this.responseContentLength)) {

...
class Parser {
  ...
  onHeadersComplete (statusCode, upgrade, shouldKeepAlive) {
    ...
    const { keepAlive, trailers, contentLength } = util.extractMeaningfulHeaders(rawHeaders)
...
function onSocketEnd () {
  if (util.mismatchContentLength(this[kParser].bodySize, this[kParser].contentLength)) {
    util.destroy(this, new ResponseContentLength())
    return
  }
...
function write (client, request) {
  ...
  if (request.contentLength !== null && request.contentLength !== contentLength) {
    ...
    if (client[kStrictContentLength]) {
      request.onError(new RequestResponseContentLength())
      assert(request.aborted)
      return false
    }

    process.emitWarning(new RequestResponseContentLength())
  }
  ...
  if (!body) {
    ...
    const onData = function (chunk) {
      try {
        ...
        if (contentLength !== null && bytesWritten + len > contentLength) {
          // @todo info, warn, error
          if (client[kStrictContentLength]) {
            util.destroy(socket, new RequestResponseContentLength())
            return
          }
    ...
    const onFinished = function (err) {
      ...
      if (!err && contentLength !== null && bytesWritten !== contentLength) {
        // @todo info, war, error
        if (client[kStrictContentLength]) {
          err = new RequestResponseContentLength()
        } else {
          process.emitWarning(new RequestResponseContentLength())
        }
      }

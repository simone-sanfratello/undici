...
function mockDispatch (opts, handler) {
  ...
  if (typeof delay === 'number' && delay > 0) {
    setTimeout(() => {
      handleReply(this[kDispatches], key)
    }, delay)
  } else {
    handleReply(this[kDispatches], key)
  }
  ...
  // @todo request
  function handleReply (mockDispatches, request) {
    let responseData
    let responseStatusCode, responseHeaders, responseTrailers
    // @todo
    if(typeof data == 'function') {
      // @todo util
      // @todo data function error
      // @todo data function async?
      // @todo invalid computed status, headers, trailers?
      // @todo pass real data to function
      const computed = data(request)
      responseData = computed.data
      if(computed.statusCode) {
        responseStatusCode = computed.statusCode 
      }
      if(computed.headers) {
        responseHeaders = {...headers, ...computed.headers} 
      }
      if(computed.trailers) {
        responseTrailers = {...trailers, ...computed.trailers} 
      }
    } else {
    ...
    handler.onHeaders(responseStatusCode, generateKeyValues(responseHeaders), resume)
    handler.onData(Buffer.from(responseData))
    handler.onComplete(generateKeyValues(responseTrailers))

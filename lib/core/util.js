...
function mismatchContentLength (bodySize, contentLength) {
  return bodySize > 0 && contentLength > 0 &&
    bodySize !== contentLength
}

function extractMeaningfulHeaders (rawHeaders) {
  const headers = {
    keepAlive: null,
    trailers: null,
    contentLength: 0
  ...
  for (let n = 0, looking = 0; n < rawHeaders.length && looking < 3; n += 2) {
    ...
    if (!headers.keepAlive && key.length === 10 && key.toString().toLowerCase() === 'keep-alive') {
      headers.keepAlive = val
      looking++
    } else if (!headers.trailers && key.length === 7 && key.toString().toLowerCase() === 'trailer') {
      headers.trailers = val
      looking++
    } else if (!headers.contentLength && key.length === 14 && key.toString().toLowerCase() === 'content-length') {
      // content-length here is always valid because lhttp validate it
      headers.contentLength = Number(val)
      looking++
    }
  }
  return headers
...
module.exports = {
  ...
  bodyLength,
  isBuffer,
  mismatchContentLength,
  extractMeaningfulHeaders
}
